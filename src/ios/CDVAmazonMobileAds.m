...
#import "CDVAmazonMobileAds.h"

static NSString * const kPluginName = @"AmazonMobileAds";
static NSString * const kCrossPlatformTool = @"CORDOVA";

@interface CDVAmazonMobileAds ()

@property AMAZONAmazonMobileAdsObjectiveCControllerImpl *controller;
...
@implementation CDVAmazonMobileAds
...
- (void)pluginInitialize {
	...
	self.controller = [AMAZONAmazonMobileAdsObjectiveCControllerImpl sharedInstance];
...
- (void)setApplicationKey:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing setApplicationKey....");
	NSData *resultJSON;
...
options:NSJSONWritingPrettyPrinted error:&error];
				resultJSON = [self.controller setApplicationKey:jsonInputArgs];
...
- (void)registerApplication:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing registerApplication....");
	NSData *resultJSON;
	resultJSON = [self.controller registerApplication:nil];
	[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];
}
- (void)enableLogging:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing enableLogging....");
	NSData *resultJSON;
	if (command.arguments) {
        	id arg = [command.arguments objectAtIndex:0];
        	if (![arg isKindOfClass:[NSDictionary class]]) {
        	...
        	if (inputDict) {	
				...
				NSData *jsonInputArgs = [NSJSONSerialization dataWithJSONObject:inputDict 
options:NSJSONWritingPrettyPrinted error:&error];
				resultJSON = [self.controller enableLogging:jsonInputArgs];
				[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];	
		}
	}	
}
- (void)enableTesting:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing enableTesting....");
	NSData *resultJSON;
	if (command.arguments) {
        	id arg = [command.arguments objectAtIndex:0];
        	if (![arg isKindOfClass:[NSDictionary class]]) {
        	...
        	if (inputDict) {	
				...
				NSData *jsonInputArgs = [NSJSONSerialization dataWithJSONObject:inputDict 
options:NSJSONWritingPrettyPrinted error:&error];
				resultJSON = [self.controller enableTesting:jsonInputArgs];
				[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];	
		}
	}	
}
- (void)enableGeoLocation:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing enableGeoLocation....");
	NSData *resultJSON;
	if (command.arguments) {
        	id arg = [command.arguments objectAtIndex:0];
        	if (![arg isKindOfClass:[NSDictionary class]]) {
        	...
        	if (inputDict) {	
				...
				NSData *jsonInputArgs = [NSJSONSerialization dataWithJSONObject:inputDict 
options:NSJSONWritingPrettyPrinted error:&error];
				resultJSON = [self.controller enableGeoLocation:jsonInputArgs];
				[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];	
		}
	}	
}
- (void)createFloatingBannerAd:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing createFloatingBannerAd....");
	NSData *resultJSON;
	if (command.arguments) {
        	id arg = [command.arguments objectAtIndex:0];
        	if (![arg isKindOfClass:[NSDictionary class]]) {
        	...
        	if (inputDict) {	
				...
				NSData *jsonInputArgs = [NSJSONSerialization dataWithJSONObject:inputDict 
options:NSJSONWritingPrettyPrinted error:&error];
				resultJSON = [self.controller createFloatingBannerAd:jsonInputArgs];
				[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];	
		}
	}	
}
- (void)createInterstitialAd:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing createInterstitialAd....");
	NSData *resultJSON;
	resultJSON = [self.controller createInterstitialAd:nil];
	[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];
}
- (void)loadAndShowFloatingBannerAd:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing loadAndShowFloatingBannerAd....");
	NSData *resultJSON;
	if (command.arguments) {
        	id arg = [command.arguments objectAtIndex:0];
        	if (![arg isKindOfClass:[NSDictionary class]]) {
        	...
        	if (inputDict) {	
				...
				NSData *jsonInputArgs = [NSJSONSerialization dataWithJSONObject:inputDict 
options:NSJSONWritingPrettyPrinted error:&error];
				resultJSON = [self.controller loadAndShowFloatingBannerAd:jsonInputArgs];
				[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];	
		}
	}	
}
- (void)loadInterstitialAd:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing loadInterstitialAd....");
	NSData *resultJSON;
	resultJSON = [self.controller loadInterstitialAd:nil];
	[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];
}
- (void)showInterstitialAd:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing showInterstitialAd....");
	NSData *resultJSON;
	resultJSON = [self.controller showInterstitialAd:nil];
	[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];
}
- (void)closeFloatingBannerAd:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing closeFloatingBannerAd....");
	NSData *resultJSON;
	if (command.arguments) {
        	id arg = [command.arguments objectAtIndex:0];
        	if (![arg isKindOfClass:[NSDictionary class]]) {
        	...
        	if (inputDict) {	
				...
				NSData *jsonInputArgs = [NSJSONSerialization dataWithJSONObject:inputDict 
options:NSJSONWritingPrettyPrinted error:&error];
				resultJSON = [self.controller closeFloatingBannerAd:jsonInputArgs];
				[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];	
		}
	}	
}
- (void)isInterstitialAdReady:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing isInterstitialAdReady....");
	NSData *resultJSON;
	resultJSON = [self.controller isInterstitialAdReady:nil];
	[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];
}
- (void)areAdsEqual:(CDVInvokedUrlCommand*)command {
	NSLog(@"Executing areAdsEqual....");
	NSData *resultJSON;
	if (command.arguments) {
        	id arg = [command.arguments objectAtIndex:0];
        	if (![arg isKindOfClass:[NSDictionary class]]) {
        	...
        	if (inputDict) {	
				...
				NSData *jsonInputArgs = [NSJSONSerialization dataWithJSONObject:inputDict 
options:NSJSONWritingPrettyPrinted error:&error];
				resultJSON = [self.controller areAdsEqual:jsonInputArgs];
				[self sendPluginResult:resultJSON callbackId:command.callbackId keepCallback:false];	
		}
	}	
}
